...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------#
    classes_path    = '/home/ZP/CLEVR_2.0/clevr_size.txt'#change
    #---------------------------------------------------------------------#
    #   anchors_path    代表先验框对应的txt文件，一般不修改。
    #   anchors_mask    用于帮助代码找到对应的先验框，一般不修改。
    #                   YoloV7-Tiny中，鉴于tiny模型对小目标的识别效果一般，
    #                   官方使用的就是[[3, 4, 5], [1, 2, 3]]，
    #                   序号为0先验框未被使用到，无需过分纠结。
    #---------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    #model_path      = 'model_data/yolov7_tiny_weights_coco.pth'#change
    model_path = ''
    #------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    save_dir            = 'logs-2-5000-nosize-fuzzy-320'#change
    #------------------------------------------------------------------#
    ...
    train_annotation_path   = '/home/ZP/CLEVR_2.0/CLEVR_size_5000_train.txt'#change
    val_annotation_path     = '/home/ZP/CLEVR_2.0/CLEVR_size_sample.txt'
    #------------------------------------------------------#
    #   设置用到的显卡
    #------------------------------------------------------#
    ...
    if True:
        ...
        if local_rank == 0:
            eval_callback   = EvalCallback(model, input_shape, anchors, anchors_mask, class_names, num_classes, val_lines, log_dir, Cuda, \
                                            map_out_path=os.path.join(save_dir,".temp_map_out"),eval_flag=eval_flag, period=eval_period)
        else:
            eval_callback   = None
